...
const C = {
  ...
  cream: '#f8f5f0', textBody: '#2d3748', textMuted: '#718096',
  border: '#e5e2dd', bgLight: '#f6f3ee', cardShadow: '0 2px 12px rgba(3,31,65,0.10)',
};

const PROMO_CONFIG = {
  dos_x_uno:       { bg: '#f59e0b', textColor: '#1a1200', icon: '🎯', badgeLabel: '2 × 1',          calloutBg: '#fef3c7' },
  precio_especial: { bg: '#059669', textColor: '#ffffff', icon: '💰', badgeLabel: 'PRECIO ESPECIAL', calloutBg: '#d1fae5' },
  cupos_limitados: { bg: '#dc2626', textColor: '#ffffff', icon: '🔥', badgeLabel: 'CUPOS LIMITADOS', calloutBg: '#fee2e2' },
  texto_libre:     { bg: '#c46b48', textColor: '#ffffff', icon: '✨', badgeLabel: '',                calloutBg: '#fef0eb' },
};
...
const SAMPLE_PLAN = {
  ...
  price: 3_500_000,
  currency: 'COP',
  priceDetails: 'por persona en habitación doble',
  durationDays: 7,
  durationNights: 6,
  departureCity: 'Bogotá',
  inclusions: ['Vuelos', 'Hotel 5★', 'Traslados', 'Seguro'],
  imageUrls: ['https://images.unsplash.com/photo-1510097467424-192d713fd8b2?w=600&h=280&fit=crop&auto=format&q=80'],
  promotion: {
    type: 'dos_x_uno',
    label: 'Viajan dos, paga uno — solo este fin de semana',
...
const SAMPLES = [
  { label: '2 × 1 (dos_x_uno)', plan: SAMPLE_PLAN },
  {
    label: 'Precio especial (precio_especial)',
    plan: {
      ...SAMPLE_PLAN,
      promotion: { type: 'precio_especial', label: '30% de descuento en la tarifa regular', expiresAt: '2025-11-15T23:59:59.000Z', active: true },
    },
  },
  {
    label: 'Cupos limitados (cupos_limitados)',
    plan: {
      ...SAMPLE_PLAN,
      promotion: { type: 'cupos_limitados', label: 'Solo quedan 4 cupos disponibles', expiresAt: undefined, active: true },
    },
  },
  {
    label: 'Texto libre (texto_libre)',
    plan: {
      ...SAMPLE_PLAN,
      promotion: { type: 'texto_libre', label: 'BLACK FRIDAY · Solo este fin de semana', expiresAt: '2025-11-30T23:59:59.000Z', active: true },
    },
  },
  {
    label: 'Sin promoción activa',
    plan: { ...SAMPLE_PLAN, promotion: undefined },
  },
];

// ─── template logic (JavaScript translation of plan-campaign.template.ts) ────
function inclusionPills(inclusions) {
  return inclusions.slice(0, 4).map(inc =>
...
function buildEmail(recipientName, plan) {
  ...
  const promoConf = promo ? (PROMO_CONFIG[promo.type] ?? PROMO_CONFIG.texto_libre) : null;

  const promoBadgeLabel = promo
    ? (promo.type === 'texto_libre' ? promo.label : promoConf.badgeLabel)
    : '';

  const imgUrl = plan.imageUrls?.[0] ?? '';
  const imgBlock = imgUrl
    ? `<img src="${imgUrl}" alt="${plan.title}" width="600" style="width:100%;height:280px;object-fit:cover;display:block">`
  ...
  const calloutTitle = promo
    ? (promo.type === 'texto_libre' ? promo.label : promoConf.badgeLabel)
    : '';
  const calloutSubtitle = promo && promo.type !== 'texto_libre' && promo.label
    ? `<p style="margin:8px 0 0;font-family:${F.sans};font-size:14px;color:${C.textBody};line-height:1.5">${promo.label}</p>`
    : '';
  ...
  const cardBadge = promoConf
    ? `<span style="display:inline-block;background:${promoConf.bg};color:${promoConf.textColor};
        ...
        padding:5px 14px;border-radius:20px;letter-spacing:0.1em;text-transform:uppercase;white-space:nowrap">
  ...
  const heroText  = hasPromo ? 'Tenemos una oferta especial para ti' : 'Una propuesta pensada para ti';
  ...
  return `
      ...
      <table cellpadding="0" cellspacing="0" width="100%">
        ...
        <tr><td style="padding:36px 40px 28px">
          <p style="margin:0 0 6px;font-family:${F.condensed};font-size:11px;font-weight:700;
            color:${C.teal};letter-spacing:0.14em;text-transform:uppercase">Hola, ${firstName}</p>
          <h1 style="margin:0 0 16px;font-family:${F.serif};font-size:32px;font-weight:700;
            color:${C.navy};line-height:1.2;font-style:italic">${heroText}</h1>
        ...
        <tr><td style="padding:0 40px 28px">
          <table cellpadding="0" cellspacing="0" width="100%"
            style="border-radius:14px;overflow:hidden;border:1px solid ${C.border};box-shadow:${C.cardShadow}">
            <tr><td style="background:${C.bgLight};padding:20px 24px 16px">
              <table cellpadding="0" cellspacing="0" width="100%"><tr>
                ...
                <td valign="top" align="right" style="padding-left:12px">${cardBadge}</td>
              </tr></table>
            </td></tr>
        ...
        <!-- SEPARATOR -->
        <tr><td style="padding:0 40px"><div style="height:1px;background:${C.border}"></div></td></tr>

        <!-- CTA -->
        <tr><td style="padding:32px 40px;text-align:center">
          <p style="margin:0 0 20px;font-family:${F.sans};font-size:14px;color:${C.textBody};line-height:1.7">
    ...
    <tr><td style="background:${C.navy};padding:24px 40px;text-align:center">
      <p style="margin:0 0 6px;font-family:${F.sans};font-size:12px;color:rgba(176,199,241,0.8);line-height:1.6">
...
// ─── build multi-variant page ─────────────────────────────────────────────────
function buildPage() {
  const sections = SAMPLES.map(({ label, plan }) => {
    const rows = buildEmail('María García', plan);
    return `
      <div style="margin:0 0 60px">
        <div style="background:#1d3557;color:#fff;padding:10px 20px;border-radius:8px 8px 0 0;
          font-family:Arial,sans-serif;font-size:13px;font-weight:700;letter-spacing:0.05em">
          VARIANTE: ${label}
        </div>
        <table width="600" cellpadding="0" cellspacing="0"
          style="max-width:600px;width:100%;border-radius:0 0 16px 16px;overflow:hidden;
...
<body style="margin:0;padding:40px 20px 80px;background:#e8e4df;font-family:Arial,sans-serif">
  <div style="max-width:640px;margin:0 auto">
    <h1 style="font-family:Arial,sans-serif;font-size:18px;color:#031f41;margin:0 0 6px">
      Preview — Plantilla de campaña promocional
    </h1>
    <p style="font-family:Arial,sans-serif;font-size:13px;color:#718096;margin:0 0 40px">
      5 variantes: una por tipo de promoción + sin promoción.
    </p>
    ${sections}
  </div>
</body>
...
const outPath = join(__dirname, 'email-preview-campaign.html');
